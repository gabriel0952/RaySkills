---
name: skill-creator
description: 幫助使用者為 skills 庫建立、改善或優化 skill。當使用者想建立一個新的 skill、把某個工作流程包裝成 skill、修改現有 skill、或優化 skill 的觸發描述時，請使用這個 skill。即使使用者只說「建立一個 skill」、「幫我包裝這個流程」、「我想把這個做成 skill」也應觸發。
---

# Skill Creator

協助建立與迭代 skills 個人 skill 庫的 skill。

## 可用工具腳本

從 `skills/skill-creator/` 目錄執行（需先 `pip install -r requirements.txt`）：
...
完成時交付：
1. 完整的 SKILL.md 草稿
2. 建議的檔案路徑（`skills/<skill-name>/SKILL.md`）
...
# 積極具體（好）
description: 幫使用者建立 skills skill。當使用者想建立新 skill、
...
**2. 執行優化迴圈**（從 `skills/skill-creator/` 執行）：
